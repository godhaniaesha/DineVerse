...
export const TableReservationProvider = ({ children }) => {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (res.status === 401 || res.status === 403 || res.status === 404) {
          lastError = data.message || data.message || `Request failed: ${endpoint}`;
          continue;
        }
        return { success: false, error: data.message || data.message || `Request failed with status ${res.status}` };
      } catch (_) {
  ...
  const updateReservationStatus = useCallback(async (id, status, waiter = null) => {
    try {
      ...
      return { success: false, error: data.message || data.message || "Failed to update status" };
    } catch (_) {
      return { success: false, error: "Network error" };
    } finally {
      setLoading(false);
