...
export default function Profile() {
  ...
  useEffect(() => {
        if (showReviewModal || showViewModal) {
  ...
  const downloadInvoice = (item) => {
    ...
    doc.autoTable({
      ...
      body: [[`Booking Reference ${item.id}`, item.service, item.amount]],
    ...
    doc.text(
      "Thank you for choosing DineVerse. We hope to see you again!",
      105,
      280,
      { align: "center" },
    );
    doc.text("12 Rue de la Paix, Paris | +1 (800) 555-NOIR", 105, 285, {
      align: "center",
    });
      ...
      case "profile":
        return (
          <div className="z_prof_section">
            <h2 className="z_prof_section_title">
          ...
          <div className="z_prof_section">
            ...
            <div className="z_prof_bookings_list">
              ...
              {bookings.map((booking) => (
                <div
                  key={booking.id || `${booking.type}-${booking.bookingRef}`}
                  className="z_prof_booking_card"
                  style={{
                    "--accent":
                      booking.type === "Room"
                        ? "var(--d-room)"
                ...
                >
                  <div className="z_prof_booking_main">
                    <div className="z_prof_booking_header">
                      <div className="z_prof_booking_title_area">
                        <h4>{booking.service}</h4>
                        {/* <button
                          className="z_prof_view_btn"
                          onClick={(e) => {
                            e.stopPropagation();
                      ...
                      <span
                        className={`z_prof_booking_status z_prof_status_${formatStatus(booking.status)}`}
                      >
                        {booking.status}
                      </span>
                    </div>
                    ...
                    <div className="z_prof_booking_meta">
                      <span>
                        <FiCalendar /> {formatBookingDate(booking.date)}
                      </span>
                      <span>
                      ...
                      {(booking.adults || booking.guests) && (
                        <span>
                          <FiUser /> {booking.adults || booking.guests} guests
                        </span>
                      )}
                      <span className="z_prof_booking_ref">
                        <FiFileText /> {booking.bookingRef || "-"}
                      </span>
                    </div>
                    <button
                          className="z_prof_view_btn"
                    ...
                    {formatStatus(booking.status) === "completed" && (
                      ...
                      >
                        ...
                        <span className="z_prof_rate_text">
  ...
  return (
    <div className="z_prof_page_wrapper">
      ...
      <div className="z_prof_container">
        ...
        {/* Main Content */}
        <main className="z_prof_content">{renderContent()}</main>
      </div>
      ...
      {showReviewModal && selectedBooking && (
        <div
          className="z_modal_overlay z_review_overlay"
          onClick={() => setShowReviewModal(false)}
        >
          <div
            className="z_modal_content z_review_content"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="z_review_header">
              <h2 className="z_modal_title">Review Your Experience</h2>
              <p className="z_review_venue">
                {selectedBooking.venue || selectedBooking.service}
              </p>
            </div>
            ...
            <div className="z_review_form">
              ...
              <div className="z_prof_field_group">
                <label className="z_prof_label">
                  Review Message
                  <span className="z_char_count">
                    {reviewMessage.length}/230
                  </span>
                </label>
                <textarea
      ...
      {/* Booking Details View Modal - Refined Luxury UI */}
      {showViewModal && selectedBooking && (
        <div
          className="z_modal_overlay z_view_overlay"
          onClick={() => setShowViewModal(false)}
        >
          <div
            className="z_view_ticket_card"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Ticket Left - Branding & Reference */}
            <div className="z_ticket_side_branding">
              <div className="z_ticket_logo">DineVerse</div>
              <div className="z_ticket_ref_wrap">
                <span className="z_ticket_ref_label">BOOKING REFERENCE</span>
                <div className="z_ticket_ref_id">#{selectedBooking.bookingRef || "DV-8829"}</div>
              </div>
              <div className="z_ticket_status_stamp">
                <span className={`z_stamp_text z_stamp_${formatStatus(selectedBooking.status)}`}>
                  {selectedBooking.status}
                </span>
              </div>
            </div>

            {/* Ticket Right - Main Content */}
            <div className="z_ticket_main_content">
              <button className="z_ticket_close" onClick={() => setShowViewModal(false)}>×</button>
              
              <div className="z_ticket_header">
                <h2 className="z_ticket_title">Reservation <span>Details</span></h2>
                {/* <p className="z_ticket_subtitle">Luxury {selectedBooking.service} Experience</p> */}
              </div>

              <div className="z_ticket_grid">
                <div className="z_ticket_item">
                  <FiCalendar className="z_ticket_icon" />
                  <div className="z_ticket_info">
                    <label>{selectedBooking.type === "Room" ? "Check-in" : "Date"}</label>
                    <span>{formatBookingDate(selectedBooking.date)}</span>
                  </div>
                </div>

                {selectedBooking.type === "Room" && (
                  <div className="z_ticket_item">
                    <FiCalendar className="z_ticket_icon" style={{ opacity: 0.6 }} />
                    <div className="z_ticket_info">
                      <label>Check-out</label>
                      <span>{formatBookingDate(selectedBooking.checkOut)}</span>
                    </div>
                  </div>
                )}

                <div className="z_ticket_item">
                  <FiClock className="z_ticket_icon" />
                  <div className="z_ticket_info">
                    <label>Time</label>
                    <span>{selectedBooking.time || "Flexible"}</span>
                  </div>
                </div>

                <div className="z_ticket_item">
                  <FiUser className="z_ticket_icon" />
                  <div className="z_ticket_info">
                    <label>Guest Details</label>
                    <span>{selectedBooking.adults || 1} Adults, {selectedBooking.children || 0} Children</span>
                  </div>
                </div>

                <div className="z_ticket_item">
                  <FiMapPin className="z_ticket_icon" />
                  <div className="z_ticket_info">
                    <label>{selectedBooking.type === "Room" ? "Room Type" : "Area"}</label>
                    <span>{selectedBooking.roomType || selectedBooking.service}</span>
                  </div>
                </div>

                <div className="z_ticket_item">
                  <FiMapPin className="z_ticket_icon" style={{ opacity: 0.6 }} />
                  <div className="z_ticket_info">
                    <label>{selectedBooking.type === "Room" ? "Room No" : "Table No"}</label>
                    <span>{selectedBooking.roomNo ? `Suite ${selectedBooking.roomNo}` : selectedBooking.tableName || "Premium Table"}</span>
                  </div>
                </div>
              </div>

              {selectedBooking.specialRequests && (
                <div className="z_ticket_requests">
                  <label>Guest Preferences</label>
                  <p>{selectedBooking.specialRequests}</p>
                </div>
              )}

              <div className="z_ticket_footer">
                <div className="z_ticket_total">
                  <label>Total Investment</label>
                  <div className="z_ticket_price">${selectedBooking.totalAmount || "0"}</div>
                </div>
                <div className="z_ticket_total text-end">
                  <label>Advance Paid</label>
                  <div className="z_ticket_price" style={{ color: "var(--d-gold)" }}>${selectedBooking.advanceAmount || "0"}</div>
                </div>
              </div>
            </div>
            
            {/* Perforated Line Decoration */}
            <div className="z_ticket_perforation"></div>
          </div>
        </div>
      )}
    </div>
