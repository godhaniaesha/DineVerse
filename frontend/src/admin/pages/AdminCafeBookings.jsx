...
export default function AdminCafeBookings({ title = "Cafe Bookings", sub = "Manage all cafe table reservations." }) {
  ...
  return (
    <div className="ad_page">
      ...
      <div className="ad_table_wrap"><table className="ad_table"><thead><tr><th>ID</th><th>Customer</th><th>Contact</th><th>Date</th><th>Time</th><th>Guests</th><th>Status</th><th>Actions</th></tr></thead><tbody>{rows.map((r) => <tr key={r.id} onClick={() => openView(r)}><td>{r.id}</td><td>{r.customer}</td><td>{r.contact}</td><td>{r.date}</td><td>{r.time}</td><td>{r.guests}</td><td><span className="ad_chip">{r.status}</span></td><td onClick={(e) => e.stopPropagation()}>
        <div className="d-flex" style={{gap:"6px"}}>
        <button className="rooms__icon_btn" onClick={() => openEdit(r)}><IcEdit /></button>
        <DeleteIconButton onClick={() => openDelete(r)} /></div></td></tr>)}</tbody></table></div>
