...
const CHEF_ROLES = new Set(["Cafe Chef", "Restaurant Chef", "Bar Chef", "Chef"]); // "Chef" pan add kari do
...
export default function AdminDishManagement() {
  ...
  // authUser ma j puro user object chhe
  let authUser = null;
  try {
  ...
  const chefId = authUser?._id || ""; // aa id use karva ni
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      // chef login hoy tyare: faqat e j dish dekhadvi jema aa chef assign chhe
      if (isChefRole) {
        const chefIds = (r.chef || []).map((c) =>
          typeof c === "string" ? c : c._id
        );
        if (!chefId || !chefIds.includes(chefId)) {
          return false;
        }
      }

      const matchArea =
        areaFilter === "All" || (r.area || []).includes(areaFilter);
  ...
  }, [rows, areaFilter, search, isChefRole, chefId]);
  ...
  const sorted = useMemo(() => {
    ...
    if (sortBy === "Course")
      return list.sort(
        (a, b) => COURSES.indexOf(a.course) - COURSES.indexOf(b.course)
      );
    return list;
  }, [filtered, sortBy]);
  ...
  if (loading)
    return (
      <div className="ad_page">
        <div className="ad_h2">Loading Dishes...</div>
      </div>
    );
    ...
    <div className="ad_page">
      <div className="rooms__header">
        <div>
          ...
          <p className="ad_p">
            Manage dishes with full course, meal, ingredient and area controls.
          </p>
        </div>
        {!isChefRole && (
          <button
          ...
          <tbody>
            {sorted.map((r) => (
              <tr key={r.id}>
                ...
                <td>{(r.area || []).join(", ")}</td>
                <td>
                  {r.chef && r.chef.length
                    ? r.chef.map((c) => c.full_name).join(", ")
                    : "-"}
                </td>
                <td>
            ...
            {sorted.length === 0 && (
              <tr>
                <td colSpan={9} className="rooms__empty">
                  No dishes found
                </td>
              </tr>
            )}
      ...
      {!isChefRole && (modal?.mode === "add" || modal?.mode === "edit") && (
        <>
          ...
          <div className="rooms__modal_box">
              ...
              <div className="d-flex align-items-center gap-2">
                ...
                {form.img && (
                  <span style={{ fontSize: "12px" }}>
                    {form.img instanceof File ? form.img.name : "Current Image"}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="rooms__form_row">
              <label className="rooms__form_label">Course</label>
              <select
                className="rooms__form_select"
                value={form.course}
                onChange={(e) =>
            ...
            <div className="rooms__form_row">
              <label className="rooms__form_label">Meal</label>
              <select
                className="rooms__form_select"
                value={form.mealType}
                onChange={(e) =>
            ...
            <div className="rooms__form_grid2">
              <div>
                <label className="rooms__form_label">Price</label>
                <input
                  type="number"
                  className="rooms__form_input"
                  value={form.price}
              ...
              <div>
                <label className="rooms__form_label">Prep time</label>
                <input
                  className="rooms__form_input"
                  value={form.prepTime}
                  onChange={(e) =>
            ...
            <div className="rooms__form_row">
              <label className="rooms__form_label">Note</label>
              <input
                className="rooms__form_input"
                value={form.note}
                onChange={(e) =>
            ...
            <div className="rooms__form_row">
              ...
              <div className="rooms__multi_select_wrapper" ref={chefDropdownRef}>
                ...
                >
                  {form.chef.length > 0 ? (
                    form.chef.map((id) => {
                      const chefObj = chefs.find((c) => c._id === id);
                      return (
                        <span key={id} className="rooms__multi_select_tag">
                          {chefObj?.full_name || id}
                        </span>
                      );
                    })
                  ) : (
                    <span className="rooms__multi_select_placeholder">
                      Select Chefs
                    </span>
                  )}
                </div>
                  ...
                  <div className="rooms__multi_select_dropdown">
                    {chefs.map((chef) => {
                      ...
                      return (
                        <div
                          key={chef._id}
                          className={`rooms__multi_select_item ${
                            isSelected ? "selected" : ""
                          }`}
                          onClick={() => {
                            const next = isSelected
                              ? form.chef.filter((cid) => cid !== chef._id)
                              : [...form.chef, chef._id];
                            setForm((f) => ({ ...f, chef: next }));
                          }}
                        >
                          <div className="rooms__multi_select_checkbox" />
                          <span className="rooms__multi_select_item_label">
                            {chef.full_name}
                          </span>
                        </div>
                      );
