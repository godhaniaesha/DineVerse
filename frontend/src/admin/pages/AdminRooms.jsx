  ...
  return (
    <div>
              ...
              .map((rt) => {
                ...
                return (
                  <option key={rt._id} value={rt._id}>
                    {label} (₹
                    {(rt.price_per_night || 0).toLocaleString("en-IN")}/night)
                  </option>
                );
              })
          )}
      ...
      <div className="rooms__form_actions">
        <button className="rooms__btn rooms__btn--ghost" onClick={onCancel}>
          Cancel
        </button>
        <button
...
export default function AdminRooms() {
  ...
  const revenue = rooms
    ...
    .reduce((s, r) => s + (r.roomType?.price_per_night || 0), 0);

  /* filtered list */
  const filtered = [...rooms].reverse().filter((r) => {
  ...
  const cycleStatus = async (room) => {
    const next =
      STATUS_CYCLE[
        (STATUS_CYCLE.indexOf(room.status) + 1) % STATUS_CYCLE.length
      ];
    await updateRoom(room._id, { status: next });
  };

  const hasFilter =
    filterType !== "All Types" || filterStatus !== "All" || search;
  const roomTypeOptions = [
    "All Types",
    ...roomTypes.map((rt) => rt.display_name || rt.name),
  ];
    ...
    <div className="rooms">
      ...
      <div className="rooms__stats">
        ...
        <div className="rooms__stat">
          ...
          <div className="rooms__stat_sub">
            {total > 0 ? Math.round((available / total) * 100) : 0}% of
            inventory
          </div>
        </div>
        <div className="rooms__stat">
        ...
        <div className="rooms__stat">
          ...
          <div className="rooms__stat_value">
      ...
      <div className="rooms__filters">
        <span className="rooms__filter_label">
          <IcFilter /> Filters
        </span>
        <input
              ...
              {loading ? (
                ...
                paginatedRooms.map((room) => (
                  <tr key={room._id} onClick={() => openView(room)}>
                    ...
                    <td className="rooms__td_price">
                      ₹
                      {(room.roomType?.price_per_night || 0).toLocaleString(
                        "en-IN",
                      )}
                    </td>
                    <td
        ...
        {Object.entries(STATUS_LABELS).map(([key, label]) => {
          ...
          return (
            <div key={key} className="rooms__legend_item">
              <span
                className={`rooms__legend_dot rooms__legend_dot--${statusClass}`}
              />
              {label}
            </div>
          );
      ...
      {(modal?.mode === "add" || modal?.mode === "edit") && (
        <Modal
          title={
            modal.mode === "add"
              ? "Add new room"
              : `Edit room #${modal.room?.roomNumber}`
          }
          onClose={closeModal}
        >
          <RoomForm
      ...
      {modal?.mode === "view" &&
        (() => {
          const r = modal.room;
          const roomTypeName =
            r.roomType?.display_name || r.roomType?.name || "-";
          return (
            <Modal
              title={`Room #${r.roomNumber} — ${roomTypeName}`}
              onClose={closeModal}
            >
              <div className="rooms__detail_grid">
                {[
                  ...
                  [
                    "Adults",
                    `${r.capacity_adults} guest${r.capacity_adults > 1 ? "s" : ""}`,
                  ],
                  [
                    "Childs",
                    `${r.capacity_childs} guest${r.capacity_childs > 1 ? "s" : ""}`,
                  ],
                  [
                    "Price / night",
                    `₹${(r.roomType?.price_per_night || 0).toLocaleString("en-IN")}`,
                  ],
