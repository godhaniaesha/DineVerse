...
export default function AdminReviews() {
  ...
  return (
    <div className="ad_page">
      ...
      <div className="ad_table_wrap"><table className="ad_table"><thead><tr><th>Name</th><th>Phone</th><th>Rating</th><th>Review</th><th>Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>{rows.map((r) => <tr key={r.id}><td>{r.name}</td><td>{r.phone_no}</td><td>{r.rating}/5</td><td>{r.message}</td><td>{r.date}</td><td><span className="ad_chip">{r.status}</span></td><td className="rooms__actions_cell"><button className="rooms__icon_btn" onClick={() => openEdit(r)}><IcEdit /></button><button className="rooms__icon_btn rooms__icon_btn--danger" onClick={() => openDelete(r)}><IcTrash /></button></td></tr>)}</tbody></table></div>
